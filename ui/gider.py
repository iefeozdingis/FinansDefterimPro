...
class GiderSayfasi(ctk.CTkFrame):
    def __init__(self, parent, db, dashboard_callback=None):
        ...
        # Ana kart
        kart = ctk.CTkFrame(
            self, corner_radius=20, fg_color="#2d0f0f", border_width=1, border_color="#c0392b"
        )
        kart.pack(pady=30, padx=40, fill="both", expand=True)

        # Başlık
        ctk.CTkLabel(
            kart, text="💸  Gider Ekle",
            font=("Segoe UI", 30, "bold"), text_color="#f87171"
        ).pack(pady=(30, 10))

        ctk.CTkLabel(
            kart, text="Yeni bir gider kaydı oluştur",
        ...
        self.kategori = ctk.CTkComboBox(
            ...
            font=("Segoe UI", 14), corner_radius=10,
            border_color="#c0392b",
            button_color="#c0392b", button_hover_color="#8b1a1a"
        )
        ...
        ctk.CTkButton(
            kart, text="💾  Gideri Kaydet",
            width=280, height=45,
            font=("Segoe UI", 15, "bold"),
            fg_color="#c0392b", hover_color="#8b1a1a",
            corner_radius=12, command=self.kaydet
        ).pack(pady=(25, 30))

    def _kategori_listesi(self) -> list:
        ozel = self.db.kategorileri_getir("Gider")
        return VARSAYILAN_GIDER_KATEGORILER + [k for k in ozel if k not in VARSAYILAN_GIDER_KATEGORILER]
